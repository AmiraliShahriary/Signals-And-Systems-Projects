...
noise_level = 0.1; % addjust the noise level 
received_with_noise = received + noise_level * randn(size(received));

% use convolution 
convolution_result = conv(received_with_noise, flip(sent));
...
xlabel('Time Delay (s)');
ylabel('Convolution');
title('Convolution Result with Noise');
hold on;
...
%  if  difference between estimated_R and true R is less than 10
diff_R = abs(estimated_R - R);
if diff_R < 10
    disp('The difference is less than 10.');
else
    disp('The difference is greater than or equal to 10.');
end
