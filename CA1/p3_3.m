
ts = 1e-9; 
T = 1e-5;  
tau = 1e-6; 

t = 0:ts:T;
tlen = length(t);
sent = zeros(1,tlen);
sent(1:round(tau/ts)) = 1;

R = 450; 
a = 0.5;
c = 3e8; 

td = 2 * R / c;

received = circshift(0.5*sent, round(td/ts));


correlation_result = xcorr(received, sent);


correlation_t = -T:ts:T;

figure;

plot(correlation_t, correlation_result);
xlabel('Time Delay (s)');
ylabel('Correlation');
title('Cross-correlation Result');
hold on;


[max_corr, max_idx] = max(correlation_result);
plot(correlation_t(max_idx), max_corr, 'ro');
legend('Correlation Result', 'Peak');


time_delay = correlation_t(max_idx);
estimated_R = time_delay * c / 2;

disp(['Estimated Distance (R): ', num2str(estimated_R), ' meters']);
